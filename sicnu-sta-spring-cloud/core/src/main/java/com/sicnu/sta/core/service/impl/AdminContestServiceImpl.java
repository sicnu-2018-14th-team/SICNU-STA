...
import com.sicnu.sta.core.service.AdminContestService;
...
public class AdminContestServiceImpl implements AdminContestService {
    ...
    public ResultUtils<Object> createContest(Contest contest) {
        try {
            contestServer.createContest(contest);
    ...
    public ResultUtils<Object> queryContestListInfo(int pageSize, int page) {
        try {
            List<Map<String, Object>> contests = contestServer.queryContestListInfo((page - 1) * pageSize, pageSize);
            for (Map<String, Object> it : contests) {
                String userName = userServer.queryUserNameByUserId((Integer) it.get("user_id"));
                it.put("user_name", userName);
            }
            List<Object> data = new ArrayList<>();
            Map<String, Integer> cnt = new HashMap<>();
            cnt.put("total", contestServer.queryContestCnt());
            data.add(cnt);
            data.add(contests);
    ...
    public ResultUtils<Object> updateContestInfo(Contest contest) {
        try {
            contestServer.updateContestInfo(contest);
    ...
    public ResultUtils<Object> addContestToClass(Integer classId, Integer contestId) {
        try {
            contestServer.addContestToClass(classId, contestId);
            // 将该比赛绑定给班级里的学生
            List<Integer> studentIds = userServer.queryClassStudents(classId);
            List<Integer> problemIds = contestProblemServer.queryProblemIdsForContest(contestId);
            for (Integer studentId : studentIds) {
                contestServer.addContestToStudent(contestId, studentId);
    ...
    public ResultUtils<Object> queryContestResult(Integer contestId, Integer classId) {
        try {
            List<Integer> userIds = contestServer.queryClassContestUserIds(contestId, classId);
            List<Map<String, Integer>> mapList = new ArrayList<>();
            List<Map<String, Object>> data = new ArrayList<>();
            System.out.println("contestId : " + contestId);
            for (Integer userId : userIds) {
                System.out.println("userId : " + userId);
                int sumScore = contestServer.queryContestUserSumScore(contestId, userId);
            ...
            for (Map<String, Integer> it : mapList) {
                ...
                lastScore = curScore;
                map.put("userName", userServer.queryUserNameByUserId(it.get("userId")));
                data.add(map);
            }
            return ResultUtils.success(data);
        } catch (Exception e) {
