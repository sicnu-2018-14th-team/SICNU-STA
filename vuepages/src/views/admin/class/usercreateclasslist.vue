  ...
  <div>
    ...
    <el-dialog title="班级比赛列表" :visible.sync="classcontestdialog" width="60%">
      <el-card>
        ...
        <el-dialog title="添加比赛" width="40%" :visible.sync="addcontestdialog" append-to-body>
          <el-card>
        ...
        <el-table :data="calsscontesttable" >
          ...
          <el-table-column label="操作" fixed="right"  >
            <template slot-scope="scope" >
              <el-tooltip class="item" effect="dark" content="查看班级排名" placement="top">
                <el-button type="success" icon="el-icon-download" @click="queryclassrank(scope.row)" size="mini"></el-button>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" content="删除比赛" placement="top">
      ...
      <el-dialog title="班级比赛排名" :visible.sync="rankdialog" width="40%" append-to-body>
        <download-excel
        ...
        <el-table :data="ranktable" stripe border  style="margin-top:10px;">
          <el-table-column type="index" label="#"></el-table-column>
...
export default {
  ...
  methods:{
    ...
    queryClassList(curry){
      ...
      axios.post(this.global.query_class_my_create,this.$qs.stringify(params)).then(function(resp){
    ...
    queryclassrank(row){
      ...
      axios.post(this.global.query_contest_result,this.$qs.stringify(pramas)).then(function(resp){
        if(resp.data.code === 200){
          _this.ranktable = resp.data.data
